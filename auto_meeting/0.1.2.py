...
class do:
    ...
    def arounder(self):
        sizex,sizey=pg.size()
        pg.moveTo(sizex-100,sizey-100,0.1,pg.easeInOutQuad)
    ...
    def quit(self):
        ...
        if a==1 or b==1:
            self.arounder()
            a=self.find_mover('quit_meet.png')
            self.find_mover('quit_sure.png')
    ...
    def entry(self):
        ...
        self.click()
        self.find_mover('del_name_num.png')
        pg.move(-25,0,1,pg.easeInOutQuad)
        #self.click()
        pc.copy(name)
        p.hotkey('ctrl','v')
    ...
    def alarm_clock(self,timer,way):
        get=time.strptime(timer,"%Y-%m-%d %H:%M:%S")
        mt=time.mktime(get)
        wait=mt-time.time()
        if wait>0:
            time.sleep(wait)
...
if __name__=='__main__':
    ...
    #a.videochange()
    #a.quit()
    a.alarm_clock('2022-10-20 11:40:11',a.test)
